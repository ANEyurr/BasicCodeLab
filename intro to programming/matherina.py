#math operators; +, -, *, /, %, //

#+ addition 
num1 = 10 
num2 = 5
sum = 10+5
print(f"The sum is: {sum}")


# - Subtraction
difference = num1 - num2
print(f"The Difference is: {difference}")

# * Multiplication
product = num1 * num2
print(f"The Product is: {product}")

# / Division
quotient = num1 / num2
print(f"The Quotient is: {quotient}")

# % Modulo = the remainder
remainder = num1 % num2
print(f"The Remainder is: {remainder}")
num3 = 83
num4 = 12
remainder2 = num3 % num4
print(f"The remainder isis: {remainder}")

# // Floor Division = forced to lowest integer
num5 = 10
num6 = 11
num7 = 12
num8 = 3
print(f"The floor is: {num5/num8}")
print(f"The floor is: {num6/num8}")
print(f"The floor is: {num7/num8}")


#-------------------------------------------------------------------------------------------------------------------------------------------------------


#addition
num1 = float(input("Please enter your first number: "))
num2 = float(input("Please enter your second number: "))
# Or to make float {num2 = float(num2)}
def addition(num1, num2):
    answer = num1 + num2
    print(answer)
addition(num1, num2)
#subtraction
num3 = float(input("Please enter your first number: "))
num4 = float(input("Please enter your second number: "))
def subtraction(num3, num4):
    answer = num3 - num4
    print(answer)
subtraction(num3, num4)   
#multiplication
num5 = float(input("Please enter your first number: "))
num6 = float(input("Please enter your first number: "))
def multiplication(num5, num6):
    answer = num5 * num6
    print(answer)
multiplication(num5, num6)
num7 = float(input("Please enter your first number: "))
num8 = float(input("Please enter your first number: "))
def division(num7, num8):
    answer = num7 / num8
    print(answer)
division(num7, num8)
